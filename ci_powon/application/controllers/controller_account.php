...
class Controller_account extends CI_Controller {
    ...
    function authenticateLogin() {
        ...
        $this->form_validation->set_rules('password', 'Password', 'trim|required|xss_clean|callback_checkUsernameAndPassword|callback_checkSuspended');
    ...
    function checkSuspended() {
        //field validation success,  validate status against database
        $username = $this->input->post('username');
        $password = $this->input->post('password');

        $isSuspended = $this->model_member->checkSuspended($username, $password);

        if($isSuspended) {
            $this->session->sess_destroy();
            $this->form_validation->set_message('checkSuspended', 'Account suspended');
            return false;
        } else {
            return true;
        }
